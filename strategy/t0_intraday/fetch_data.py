"""通过项目内 scripts/fdata.py 获取分钟级 K 线并缓存为 CSV.

所有数据一律走 fdata.py CLI (JSON 输出), 保证与项目其它模块同源同口径。
...
def fdata_kline(code: str, period: str = "5m", kind: str = "stock") -> pd.DataFrame:
    """调用 fdata.py kline 子命令 (--limit 0 = 全历史), 返回升序 DataFrame."""
    proc = subprocess.run(
        [str(PY), str(FDATA), "kline", code, "--kind", kind,
         "--period", period, "--limit", "0"],
        cwd=str(REPO), capture_output=True, text=True, timeout=300,
    )
    if proc.returncode != 0:
        raise RuntimeError(f"fdata kline 失败: {proc.stderr[-500:]}")
