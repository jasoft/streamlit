...
import pandas as pd
from data_analysis.stockview.log import logger
import pytz
import os
import sys
import requests
...
from data_analysis.stockview.index_spread import create_spread_chart
from data_analysis.stockview.index_comparison import render_index_comparison_page
from data_analysis.stockview.if_im_strategy import render_if_im_strategy_page
from data_analysis.stockview.akcache.rate_limiter import rate_limiter
...
from data_analysis.stockview.db_helper import get_intraday_curve, save_snapshot
...
def get_n_day_avg_amount(n: int) -> int:
    """获取最近n个交易日的平均成交额（使用通达信协议历史数据）

    通达信指数日K自带每日成交额（元），直接取最近n日均值，
    与实时成交额（腾讯源，单位元）口径一致。
    """
    ...
    try:
        ...
        sh_df = fetch_index_daily("sh000001", count=n + 5)
        sz_df = fetch_index_daily("sz399001", count=n + 5)

        if sh_df.empty or sz_df.empty:
            logger.warning("通达信源返回空数据，返回实时成交额")
            sh_amount, sz_amount = get_a_amount()
            return int(sh_amount + sz_amount)

        # 多取5根兜底除权/停市边界, 取最近n个交易日
        sh_avg_amount = sh_df["amount"].astype(float).tail(n).mean()
        sz_avg_amount = sz_df["amount"].astype(float).tail(n).mean()

        total_avg = int(sh_avg_amount + sz_avg_amount)

        logger.info(f"最近{n}日平均成交额: {total_avg/1e8:.0f}亿 (基于通达信源历史数据)")
        return total_avg
...
def streamlit_app():
    ...
    with tab1:
        ...
        if data is None:
            ...
            from data_analysis.stockview.db_helper import get_snapshot_history
