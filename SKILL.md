...
### 4.1.x worktree 多开（随机端口 + 自动回收）

多 worktree 并行开发时用 `dev.sh` 的 worktree 子命令，各栈自动分配随机端口（后端 20000+ / 前端 21000+ / fdata 22000+ 段），互不冲突：

```bash
./dev.sh worktree new fix-demo            # 创建 ../streamlit-fix-demo + uv sync + npm ci + 后台启动(随机端口)
./dev.sh worktree up [name]               # 后台启动已有 worktree（在 worktree 内可省略 name）
./dev.sh worktree down [name]             # 停止
./dev.sh worktree ls                      # 查看注册栈（●运行中 / ○已停止 / ⚠已合并 / ✗已删除）
./dev.sh worktree clean                   # 回收"已删除/已合并回主干"栈的残留进程与注册信息
./dev.sh worktree clean -a                # 连仍在运行的栈一起全部停掉
```

机制要点:

- 注册表在**主 `.git/dev-worktrees/`**（所有 worktree 共享，不随 worktree 删除丢失）；
- 在 linked worktree 里直接跑 `./dev.sh` 也会自动进入 worktree 模式（随机端口 + 注册，Ctrl-C 停止并注销）；
- **自动回收**: worktree 目录被删除或分支合并回 main 后，下次任意一处运行 `./dev.sh` 会自动 kill 其残留服务并 `git worktree prune`（按端口 + 进程 cwd 精准匹配，不误杀其他 worktree；squash 合并的分支不自动识别，用 `clean <name>` 手动回收）；
- 前端经 `next.config.ts` 的 rewrite 访问后端，worktree 模式由 dev.sh 注入 `BACKEND_ORIGIN=http://localhost:<随机后端端口>`（缺省仍为 `http://localhost:8000`），后端经 `FDATA_PORT` 环境变量连自己的 fdata；
- 新 worktree 会自动从当前树复制 `.env`；`SKIP_SETUP=1` 可跳过依赖安装。
